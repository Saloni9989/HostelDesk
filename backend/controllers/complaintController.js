...
const getComplaints = async (req, res) => {
  try {
    ...
    const conditions = ['c.is_spam = 0'];
    const filterParams = []; // params for WHERE clause only

    if (myComplaints === 'true') {
      conditions.push('c.user_id = ?');
      filterParams.push(req.user.id);
    } else if (req.user.role === 'student') {
      // students see their own + all non-rejected
    ...
    const sql = `
      SELECT c.*,
        ...
        r.stars AS rating,
        MAX(CASE WHEN uv2.user_id = ${parseInt(req.user.id)} THEN 1 ELSE 0 END) AS user_voted
      FROM complaints c
      JOIN users u ON c.user_id = u.id
      LEFT JOIN upvotes uv ON uv.complaint_id = c.id
      LEFT JOIN upvotes uv2 ON uv2.complaint_id = c.id AND uv2.user_id = ${parseInt(req.user.id)}
      LEFT JOIN comments cm ON cm.complaint_id = c.id
      LEFT JOIN ratings r ON r.complaint_id = c.id
      ${whereClause}
      GROUP BY c.id
      ORDER BY ${sortColumn === 'votes' ? 'COUNT(DISTINCT uv.id)' : `c.${sortColumn}`} ${orderDir}
      LIMIT ${parseInt(limit)} OFFSET ${offset}
    `;

    // only filter params needed now (no subquery params, no limit/offset params)
    const mainParams = [...filterParams];
    const countParams = [...filterParams];

    const [complaints, countResult] = await Promise.all([
      query(sql, mainParams),
      query(`SELECT COUNT(*) AS total FROM complaints c ${whereClause}`, countParams),
    ]);
...
// ── PATCH /api/complaints/:id/status (owner self-update) ─────
const updateOwnStatus = async (req, res) => {
  try {
    ...
    if (!allowed.includes(status)) {
      return res.status(400).json({ error: `You can only set status to: ${allowed.join(', ')}` });
    }
    ...
    if (['Resolved', 'Rejected', 'Withdrawn'].includes(complaint.status)) {
      return res.status(400).json({ error: `Cannot update a complaint that is already ${complaint.status}` });
    }
    ...
    console.error('updateOwnStatus error:', err);
    res.status(500).json({ error: 'Failed to update status' });
  }
};

module.exports = { getComplaints, getComplaintById, createComplaint, upvoteComplaint, addComment, rateComplaint, updateOwnStatus };
